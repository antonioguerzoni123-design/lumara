...
/**
 * Fetch a bundle product from Shopify Admin REST API (bypasses Storefront channel restrictions).
 * Returns shape compatible with ShopifyBundleProduct in BundleDetail.tsx, or null on failure.
 */
export async function getAdminBundleProduct(handle) {
  try {
    const data = await adminFetch(`/products.json?handle=${encodeURIComponent(handle)}&limit=1`, {
      cache: 'no-store',
    });
    ...
    variants.forEach((v) => {
      ...
      shopifyVariantIds[v.title] = gid;
      shopifyVariantPrices[v.title] = parseFloat(v.price);
    });

    const availableVariantTitles = variants
      .filter((v) => v.inventory_quantity > 0 || v.inventory_management === null)
      .map((v) => v.title);
    ...
    return {
      ...
      variants: variants.length > 1 ? availableVariantTitles : null,
      _shopifyVariantIds: shopifyVariantIds,
      _shopifyVariantPrices: shopifyVariantPrices,
      _defaultVariantId: variants[0] ? `gid://shopify/ProductVariant/${variants[0].id}` : null,
    };
  } catch {
    return null;
  }
