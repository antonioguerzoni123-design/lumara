...
function mapShopifyProduct(node, local) {
  ...
  // Variantes visíveis: apenas quando há mais de uma
  const shopifyVariantTitles = variantEdges.length > 1
    ? variantEdges.filter((e) => e.node.availableForSale).map((e) => e.node.title)
    : undefined;

  // Resolver defaultVariant: preferir o nome local se existir no Shopify; senão, primeira disponível
  const resolvedDefaultVariant = (local?.defaultVariant && shopifyVariantIds[local.defaultVariant])
    ? local.defaultVariant
    : (shopifyVariantTitles?.[0] ?? local?.defaultVariant);

  // Edge da variante padrão — garante preço e ID consistentes com o que ProductDetail mostra ao abrir
  const defaultVariantEdge = resolvedDefaultVariant
    ? (variantEdges.find((e) => e.node.title === resolvedDefaultVariant)
       ?? variantEdges.find((e) => e.node.availableForSale)
       ?? variantEdges[0])
    : (variantEdges.find((e) => e.node.availableForSale) ?? variantEdges[0]);

  // Preço = preço da variante padrão (igual ao displayPrice no ProductDetail ao abrir)
  const price = parseFloat(
    defaultVariantEdge?.node.priceV2?.amount ?? node.priceRange.minVariantPrice.amount
  );
  const compareAtRaw = parseFloat(
    defaultVariantEdge?.node.compareAtPriceV2?.amount
    ?? node.compareAtPriceRange?.minVariantPrice?.amount
    ?? '0'
  );
  const hasCompare = compareAtRaw > price;
  ...
  return {
    ...
    noReturns:            local?.noReturns,
    bestValueVariant:     local?.bestValueVariant,
    featuredPromoVariant: local?.featuredPromoVariant,

    price,
    originalPrice: hasCompare ? compareAtRaw : local?.originalPrice,
    discount:      hasCompare ? Math.round((1 - price / compareAtRaw) * 100) : local?.discount,

    _shopifyVariantIds:    shopifyVariantIds,
    _shopifyVariantPrices: shopifyVariantPrices,
    _defaultVariantId:     defaultVariantEdge?.node.id ?? null,
  };
}
